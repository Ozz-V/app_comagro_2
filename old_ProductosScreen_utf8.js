...
// Extrae el primer n├║mero de un string para comparaci├│n de specs
// Ej: "150 L/min" ÔåÆ 150, "3.5 HP" ÔåÆ 3.5, "N/A" ÔåÆ null
...
// Cach├® AsyncStorage ÔÇö igual que el HTML (4 horas)
...
function esValorValido(val) {
  ...
  const basura = ['n/a','na','n.a','n.a.','no aplica','sin dato','sin datos',
    'no','no tiene','no disponible','pim','-','--','---','st','sin informaci├│n',
...
export default function ProductosScreen({ navigation, route }) {
  ...
  const [bgActualiz, setBgActualiz]   = useState(false); // actualizaci├│n silenciosa
  const [bgBannerVisible, setBgBannerVisible] = useState(false);
  ...
  // Estados modales y navegaci├│n y precarga
  const [generandoPdf, setGenerandoPdf]   = useState(false);
  const [compartiendo, setCompartiendo]   = useState(false);
  const [pdfCache, setPdfCache]           = useState({ prodBase64: '', logoBase64: '' });
  const [logoRefreshKey, setLogoRefreshKey] = useState(''); // Se cargar├í de AsyncStorage
  ...
  // Productos similares (misma subcategor├¡a, diferente modelo)
  ...
  // ÔöÇÔöÇÔöÇ ANALYTICS: registrar acciones en Supabase y Local ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  async function trackAnalytics(prod, action) {
    try {
      let userEmail = 'offline_user';
      try {
        const cached = await AsyncStorage.getItem('@user_profile_cache');
        if (cached) userEmail = JSON.parse(cached).email || 'offline_user';
      } catch (e) {}
      ...
      // 2. Actualizar recientes y tendencias local instant├íneo
      if (action === 'view') {
        const rec = await AsyncStorage.getItem('@portal_recientes');
        let recientes = rec ? JSON.parse(rec) : [];
        recientes = [newEvent, ...recientes.filter(r => r.sku !== newEvent.sku)].slice(0, 5);
        await AsyncStorage.setItem('@portal_recientes', JSON.stringify(recientes));
      }
      
      try {
        const tStr = await AsyncStorage.getItem('@portal_tendencias');
        if (tStr) {
          const t = JSON.parse(tStr);
          let list = null;
          if (action === 'view') list = t.vistos;
          else if (action === 'search') list = t.buscados;
          else if (action.startsWith('share')) list = t.compartidos;
          
          if (list) {
            const existing = list.find(x => (x.sku || x.modelo) === (newEvent.sku || newEvent.modelo));
            if (existing) {
              existing.count = (existing.count || 0) + 1;
            } else {
              list.push({ ...newEvent, count: 1 });
            }
            list.sort((a, b) => b.count - a.count);
            await AsyncStorage.setItem('@portal_tendencias', JSON.stringify(t));
          }
        }
      } catch(e) {}
      ...
      // 3. Intentar subir
      if (user) {
        const { error } = await supabase.from('producto_analytics').insert(newEvent);
        if (!error) {
          // Remover de la cola si subi├│ bien
          const currQ = await AsyncStorage.getItem('@analytics_queue');
          if (currQ) {
            const parsed = JSON.parse(currQ).filter(item => item.created_at !== newEvent.created_at);
            await AsyncStorage.setItem('@analytics_queue', JSON.stringify(parsed));
          }
        }
      }
    } catch (e) {
      // Offline: se queda en la cola
    }
  }

  // Abrir otro producto desde la pesta├▒a Similares
  ...
  // Cargar datos de IA y precarga PDF cuando se abre modal
  useEffect(() => {
    if (activeTab === 'ASISTENTE' && modalProd?.modelo) {
      fetchAiData(modalProd.modelo);
    }
    
    // Precarga invisible de im├ígenes a Base64 para el PDF
    if (modalProd) {
      let isCurrent = true;
      ...
      const preload = async () => {
        try {
          ...
          const logoP = fetchImageBase64(logoUrl);
          const prodP = fetchImageBase64(modalProd.imagen);
          
          const logoB64 = await Promise.race([logoP, timeoutPromise]).catch(() => '');
          const prodB64 = await Promise.race([prodP, timeoutPromise]).catch(() => '');
          
          if (isCurrent) {
            setPdfCache({ logoBase64: logoB64, prodBase64: prodB64 });
          }
        } catch(e) {}
      };
      preload();
  ...
  }, [activeTab, modalProd, logoRefreshKey]);

  async function fetchAiData(sku) {
    if (!sku) {
      setAiData('Texto inteligente en preparaci├│n para este producto.');
      return;
    }
    setLoadingAi(true);
  ...
  // Precarga silenciosa de TODOS los textos de IA para uso offline
  async function preloadAiData(currentProducts) {
    try {
      const { data } = await supabase.from('productos_ai_data').select('sku, sales_pitch');
      let aiDict = {};
      if (data && data.length > 0) {
        data.forEach(item => { if(item.sales_pitch) aiDict[item.sku] = item.sales_pitch; });
        await AsyncStorage.setItem('@ai_cache_all', JSON.stringify(aiDict));
      }
      
      // Correr el robot autom├ítico de IA en segundo plano solo si recibimos la lista de productos
      if (currentProducts && currentProducts.length > 0) {
        runAutoAiGenerator(currentProducts, aiDict);
      }
    } catch (_) {}
  }

  // Generador autom├ítico en segundo plano (L├¡mite: 5 productos diarios por dispositivo)
  async function runAutoAiGenerator(productos, aiDict) {
    try {
      const today = new Date().toDateString();
      const lastRun = await AsyncStorage.getItem('@ai_last_auto_gen');
      if (lastRun === today) return; // Ya corri├│ hoy, no hacemos nada

      // Filtrar los que no tienen texto (excluyendo ceros o nulls)
      const faltantes = productos.filter(p => p.modelo && !aiDict[p.modelo]);
      if (faltantes.length === 0) {
        await AsyncStorage.setItem('@ai_last_auto_gen', today);
        return;
      }
      await AsyncStorage.setItem('@ai_last_auto_gen', today);
    } catch (_) {}
  ...
  // Cierra el modal de manera inteligente: vuelve al dashboard si entramos directo
  ...
  // ÔöÇÔöÇÔöÇ CARGA INTELIGENTE (SMART MERGE) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  // Fase 1: muestra cach├® instant├íneamente
  // Fase 2: fetch en background ÔåÆ merge de nuevos/modificados por SKU
  async function cargarDatos(forzar = false) {
    setError(null);

    // FASE 1 ÔÇö mostrar cach├® al instante
    ...
    if (rawCacheado) {
      ...
      // Si el cach├® es reciente Y no se forz├│ refresh, no ir a la red
      const cacheVigente =
        fechaCache &&
      ...
      // Cach├® existe pero venci├│ (o se forz├│): actualizar en background
      setBgActualiz(true);
    }
    // Si no hay cach├® en absoluto, mostrar spinner normal
    else {
      setCargando(true);
    }

    // FASE 2 ÔÇö fetch (Edge Function es smart: devuelve nuevos/modificados)
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const headers = { Authorization: `Bearer ${session.access_token}` };

      // Pasar timestamp del ├║ltimo fetch para que el Edge sepa qu├® devolver
      if (fechaCache && !forzar) {
        headers['X-Since'] = fechaCache;
      }

      const res = await fetch(EDGE_URL, { headers });
      if (!res.ok) throw new Error(await res.text() || 'Error en conexi├│n');
      const nuevosRows = await res.json();

      // Hacer merge con el cach├® existente: actualizar/agregar por SKU
      let rowsBase = [];
      if (rawCacheado) {
      ...
      // Tras procesar los datos nuevos, actualizamos la IA en background
      preloadAiData(procesadosFinal);
      return procesadosFinal;
    } catch (e) {
      // Ya mostramos el cach├® (si exist├¡a), solo loguear el error
      if (!rawCacheado) {
  ...
  function onRefresh() {
    ...
    setLogoRefreshKey(newKey); // forzar limpieza de cach├® de im├ígenes
    AsyncStorage.setItem('@logo_refresh_key', newKey).catch(()=>{});
    cargarDatos(true); // forzar fetch ignorando cach├®
  }
  ...
  // ÔöÇÔöÇÔöÇ GENERAR HTML FICHA ÔÇö SIEMPRE 1 HOJA A4 ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  // ESTRUCTURA: imagen+nombre SIEMPRE arriba, specs SIEMPRE abajo.
  // COLUMNAS: siempre 1 columna. Si la imagen quedar├¡a < 80px, fallback a 2 cols.
  // La imagen se achica/agranda seg├║n el espacio que sobra tras los specs.
  function generarHtmlFicha(specs, base64Img, logoBase64) {
    const numSpecs = specs.length;

    // ÔöÇÔöÇ Constantes de p├ígina (px a 96dpi, A4 portrait = 297mm Ôëê 1122px) ÔöÇÔöÇÔöÇÔöÇÔöÇ
    const PX_PAGE     = 1108; // alto total usable
    const PX_HDR      = 96;   // header (logo + l├¡nea verde + padding)
    const PX_TMARG    = 12;   // margin-top del bloque superior
    const PX_GAP      = 12;   // espacio entre bloque superior y specs
    const PX_SPEC_HDR = 34;   // cabecera azul "ESPECIFICACIONES T├ëCNICAS"
    const PX_FOOT     = 34;   // footer
    const PX_SAFETY   = 14;   // margen de seguridad
    const MIN_IMG_H   = 80;   // altura m├¡nima aceptable de la imagen
    const TEXT_H      = 77;   // altura aproximada del bloque de texto debajo de imagen

    // ÔöÇÔöÇ Intentar 1 columna ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    // Font y padding seg├║n densidad (1 col)
    ...
    // ÔöÇÔöÇ Decidir si usar 1 o 2 columnas ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    let doubleCols, topBlockH, specFs, specPad;

    if (imgH1 >= MIN_IMG_H) {
      // Ô£à 1 columna: la imagen entra bien
      doubleCols = false;
      topBlockH  = Math.max(MIN_IMG_H + TEXT_H, topH1);
      specFs     = fs1;
      specPad    = pad1;
    } else {
      // ÔÜá´©Å Fallback: 2 columnas (mitad de filas ÔåÆ m├ís espacio para imagen)
      doubleCols = true;
    ...
    // ÔöÇÔöÇ Generar filas HTML ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    let rowsHtml = '';
    if (doubleCols) {
    ...
    const specsHtml = numSpecs > 0
      ? `<div class="stitle">ESPECIFICACIONES T├ëCNICAS</div>
  ...
  async function compartirPdf() {
    try {
      ...
      // Si el vendedor presiona el bot├│n antes de que termine la precarga de fondo
      if (!finalProdB64) {
        const marcaSlug = modalProd?.marca?.toUpperCase().replace(/\s+/g, '_') || '';
        const logoUrl = `https://www.chacomer.com.py/media/wysiwyg/comagro/brands2025/${marcaSlug}.jpg?v=${logoRefreshKey}`;
      ...
      const htmlContent = generarHtmlFicha(specs, finalProdB64, finalLogoB64);
  ...
  // ÔöÇÔöÇÔöÇ COMPARTIR COMO IMAGEN (DESDE HTML A4) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  async function compartirImagen() {
    try {
      setCompartiendo(true);
      const isAvailable = await Sharing.isAvailableAsync();
      if (!isAvailable) {
        showAlert('Error', 'Compartir no est├í disponible en este dispositivo');
      ...
      showAlert('Error', 'No se pudo preparar la ficha. Intent├í de nuevo.');
      setCompartiendo(false);
    }
  }

  async function capturarHtmlOculto() {
    try {
      await new Promise(resolve => setTimeout(resolve, 800)); // tiempo para renderizar
  ...
  // ÔöÇÔöÇÔöÇ FILTRADO ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  // ÔöÇÔöÇÔöÇ RENDERS ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
        ...
        onPress={() => {
          if (isComparing) {
            if (isSelected) {
              ...
              showAlert('L├¡mite', 'Pod├®s comparar hasta 4 productos a la vez.');
              return;
            } else {
  ...
  }, [cardW, isComparing, compareItems, busqueda]);

  const mostrarLista = filtroMarca || busqueda.trim();

  // ÔöÇÔöÇÔöÇ RENDER PRINCIPAL ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ

  return (
    <SafeAreaView style={styles.safe}>
      <StatusBar backgroundColor={COLORS.white} barStyle="dark-content" />

      { (!openedDirectlyRef.current && !isInitializingDirect) ? (
        <>
          {/* Topbar */}
        ...
        <View style={styles.topbarHeader}>
          ...
          <TouchableOpacity onPress={() => {
            ...
            <Text style={styles.btnVolver}>
              {(filtroMarca || busqueda) ? 'ÔÇ╣ Volver a marcas' : 'ÔÇ╣ Volver'}
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          <View style={[styles.searchWrap, { flex: 1, marginBottom: 0 }]}>
            ...
            <TextInput
              ...
              placeholder={filtroMarca ? `Buscar en ${filtroMarca}ÔÇª` : 'Buscar productoÔÇª'}
              placeholderTextColor={COLORS.gray4}
              value={busqueda}
              onChangeText={v => {
                setBusqueda(v);
                if (v && !filtroMarca) {
                  // B├║squeda global: limpiar filtros de marca/subcategor├¡a
            ...
            {busqueda ? (
              <TouchableOpacity onPress={() => setBusqueda('')}>
                <Text style={styles.clearBtn}>Ô£ò</Text>
              </TouchableOpacity>
            ) : null}
          </View>
        </View>
      ...
      {cargando ? (
        <View style={styles.center}>
          ...
          <Text style={styles.centerText}>Cargando cat├ílogoÔÇª</Text>
        </View>
      ) : error ? (
        <View style={styles.center}>
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <Text style={styles.welcomeText}>Seleccion├í una marca para ver los productos disponibles.</Text>
          }
        ...
        productosFiltrados.length === 0 ? (
          <View style={styles.center}>
            <Text style={styles.centerText}>Sin resultados para esta b├║squeda</Text>
          </View>
        ) : (
          <>
            {/* Indicador sutil de actualizaci├│n en background */}
            {bgActualiz && (
              <View style={styles.bgBanner}>
                <ActivityIndicator size="small" color={COLORS.white} />
                <Text style={styles.bgBannerText}>Actualizando cat├ílogoÔÇª</Text>
              </View>
            )}
          <FlatList
      ...
      {isComparing && (
        <View style={{ position: 'absolute', bottom: 20 + (insets.bottom || 0), left: 20, right: 20, backgroundColor: COLORS.white, padding: 15, borderRadius: 12, elevation: 5, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1, borderColor: COLORS.border }}>
          ...
          <View style={{ flexDirection: 'row', gap: 10 }}>
            ...
            <TouchableOpacity 
              onPress={() => { if(compareItems.length > 1) setShowCompareGrid(true); else showAlert('Aviso', 'Seleccion├í al menos 2 productos para comparar.'); }}
        ...
        <View style={styles.modalOverlay}>
          ...
          {prevProd && (
            <TouchableOpacity onPress={() => handleOpenModal(prevProd)} style={{ position: 'absolute', left: 5, top: '50%', zIndex: 999, backgroundColor: 'rgba(0,0,0,0.3)', padding: 10, borderRadius: 30 }}>
              <Text style={{ fontSize: 40, color: COLORS.white, fontWeight: 'bold' }}>ÔÇ╣</Text>
            </TouchableOpacity>
          )}
          ...
          <View style={[styles.modalDialog, { paddingBottom: insets.bottom || 15 }]} {...panResponder.panHandlers}>
            <View style={styles.modalHead}>
              ...
              <TouchableOpacity onPress={cerrarModal} style={{ marginLeft: 15, padding: 5 }}>
                <Text style={styles.modalClose}>Ô£ò Cerrar</Text>
              </TouchableOpacity>
            </View>
              {/* TABS DE NAVEGACI├ôN */}
              ...
              <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
                ...
                {/* PESTA├æA FICHA T├ëCNICA */}
                {activeTab === 'FICHA' && (
                  <View>
                    {/* VISTA M├ôVIL (Lo que ve el usuario en pantalla) */}
                    <View style={{ backgroundColor: COLORS.white, padding: 15, borderRadius: 8 }}>
                      ...
                      <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', marginBottom: 10 }}>
                        ...
                        <Text style={{ fontFamily: FONTS.heading, fontSize: 16, color: '#0a2566', letterSpacing: 1 }}>FICHA T├ëCNICA</Text>
                      </View>
                      ...
                      {modalProd?.specs?.length > 0 && (
                        <View style={styles.specsWrap}>
                          <View style={styles.specsHead}>
                            <Text style={styles.specsHeadText}>Especificaciones t├®cnicas</Text>
                          </View>
                    ...
                    {/* Botones de acci├│n (PDF e Imagen) */}
                ...
                {activeTab === 'ASISTENTE' && (
                  <View style={styles.tabContent}>
                    ...
                    {loadingAi ? (
                      ...
                      <Text style={styles.aiBodyText}>{aiData || 'Texto inteligente en preparaci├│n para este producto.'}</Text>
                    )}
                    {aiData && aiData !== 'Texto inteligente en preparaci├│n para este producto.' && (
                      <TouchableOpacity 
                ...
                {/* PESTA├æA PRODUCTOS SIMILARES */}
                {activeTab === 'SIMILARES' && (
                  <View style={styles.tabContent}>
                    {/* Misma marca ÔÇö scroll horizontal */}
                    {productosMismaMarca.length > 0 && (
                      <View style={{ marginBottom: 16 }}>
                        <Text style={styles.simSectionTitle}>M├ís de {modalProd?.marca}</Text>
                    ...
                    {productosSimilares.length > 0 && (
                      <View>
                        <Text style={styles.simSectionTitle}>Misma categor├¡a</Text>
            ...
            {/* VISTA OCULTA PARA CAPTURA DE IMAGEN (Id├®ntica al PDF) */}
              ...
              <View ref={fichaRef} collapsable={false} style={{ width: 800, backgroundColor: '#fff', paddingBottom: 40, paddingTop: 30 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 45, marginBottom: 15 }}>
                  <View style={{ width: 325, alignItems: 'center', justifyContent: 'center', marginRight: 20 }}>
                    <Image source={{ uri: `${LOGO_BASE}${(modalProd?.marca||'').toUpperCase().replace(/\s+/g,'_')}.jpg?v=${logoRefreshKey}` }} style={{ width: 325, height: 180 }} resizeMode="contain" />
                  </View>
                  <View style={{ width: 2, height: 90, backgroundColor: '#a0a0a0', marginHorizontal: 30 }} />
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontFamily: FONTS.heading, fontSize: 32, fontWeight: 'bold', color: '#0a2566', letterSpacing: 1 }}>FICHA T├ëCNICA</Text>
                  ...
                  {modalProd?.specs?.length > 0 && (
                    <View style={{ width: '100%', marginBottom: 60 }}>
                      <View style={{ backgroundColor: '#0a2566', paddingVertical: 12, paddingHorizontal: 20 }}>
                        <Text style={{ color: 'white', fontSize: 18, fontWeight: 'bold', letterSpacing: 0.5 }}>Especificaciones t├®cnicas</Text>
                      </View>
                      {modalProd.specs.map(([n, v], i) => (
      ...
      {/* Modal de Comparaci├│n Inteligente */}
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.bg }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 15, backgroundColor: COLORS.white, borderBottomWidth: 1, borderColor: COLORS.border }}>
            ...
            <TouchableOpacity onPress={() => setShowCompareGrid(false)}>
              <Text style={{ fontSize: 24, color: COLORS.gray4 }}>Ô£ò</Text>
            </TouchableOpacity>
          </View>

          <ScrollView contentContainerStyle={{ padding: 12 }}>
            {/* Cabecera con im├ígenes y nombres */}
            <View style={{ flexDirection: 'row', marginBottom: 10 }}>
              ...
              {compareItems.map((prod, idx) => (
                ...
                >
                  ...
                  >
                    <Text style={{ fontSize: 10, color: COLORS.navy, fontWeight: 'bold' }}>­ƒöä Cambiar</Text>
                  </TouchableOpacity>
                </TouchableOpacity>
              ))}
            </View>

            {/* Tabla de specs con indicadores ÔåæÔåô */}
            {(() => {
              // Reunir todos los nombres de specs ├║nicos en orden
              const allSpecNames = [];
              const BLACKLIST_SPECS = ['nombre del producto', 'denominador est├índar', 'denominador estandar', 'sku', 'accesorios', 'aplicaci├│n', 'aplicacion', 'descripci├│n', 'descripcion', 'marca', 'modelo', 'imagen', 'video', 'id'];
              ...
              return allSpecNames.map((specName, si) => {
                ...
                const COMPARABLE_SPECS = ['caudal', 'tensi├│n', 'potencia', 'kva', 'presi├│n', 'capacidad', 'cilindrada', 'peso', 'velocidad', 'rpm', 'amper', 'voltaje', 'altura', 'fuerza', 'torque', 'fases'];
                ...
                return (
                  <View key={specName} style={{ flexDirection: 'row', backgroundColor: si % 2 === 0 ? '#F7F8FA' : COLORS.white, borderRadius: 6, marginBottom: 2, paddingVertical: 6, alignItems: 'center' }}>
                    ...
                    {compareItems.map((prod, pi) => {
                      ...
                      let indicator = null;
                      if (hasDiff && num !== null) {
                        if (num === maxNum) indicator = <Text style={{ color: '#16a34a', fontWeight: 'bold', fontSize: 12 }}> Ôåæ</Text>;
                        else if (num === minNum) indicator = <Text style={{ color: '#dc2626', fontWeight: 'bold', fontSize: 12 }}> Ôåô</Text>;
                      }
                      return (
          ...
          <View style={{ backgroundColor: COLORS.white, borderRadius: 15, maxHeight: '80%', overflow: 'hidden' }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 15, borderBottomWidth: 1, borderColor: COLORS.border }}>
              ...
              <TouchableOpacity onPress={() => setShowReplaceSelector(false)}>
                <Text style={{ fontSize: 20, color: COLORS.gray4 }}>Ô£ò</Text>
              </TouchableOpacity>
            </View>
      ...
      {htmlForImage && (
        <View style={{ position: 'absolute', top: 0, left: 0, width: 794, height: 1123, zIndex: -10, opacity: 0.01 }} pointerEvents="none" collapsable={false} ref={hiddenWebViewRef}>
          <WebView 
...
const styles = StyleSheet.create({
  ...
  // Marcas
  marcasGrid: { padding: 12, paddingBottom: 100 },
  ...
  // Productos grid
  prodGrid: { padding: 10, paddingBottom: 100 },
  ...
  // Banner actualizaci├│n en background
  bgBanner: {
