...
export default function LottieSplashScreen({ onFinish, updateState, updateNotes, downloadProgress, onAccept, onDecline, onInstall }) {
  ...
  useEffect(() => {
    // Solo hacer fade out si NO hay actualización pendiente o en curso
    if (updateState === 'checking' || updateState === 'prompt' || updateState === 'downloading' || updateState === 'ready') return;
  ...
  else if (updateState === 'prompt') statusText = '';
  ...
  return (
    <Animated.View style={[styles.container, { opacity: fadeAnim }]}>
      ...
      <View style={styles.bottom}>
        {/* Prompt de actualización premium */}
        {updateState === 'prompt' && (
          <View style={styles.promptCard}>
            <Text style={styles.promptTitle}>Nueva actualización disponible</Text>
            <Text style={styles.promptNotes}>{updateNotes || 'Nuevas mejoras y correcciones generales'}</Text>
            <View style={styles.promptButtons}>
              <TouchableOpacity style={styles.declineBtn} onPress={onDecline} activeOpacity={0.7}>
                <Text style={styles.declineText}>Más tarde</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.acceptBtn} onPress={onAccept} activeOpacity={0.7}>
                <Text style={styles.acceptText}>Actualizar</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
