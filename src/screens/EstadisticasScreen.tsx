...
export default function EstadisticasScreen({ navigation }: { navigation: { navigate: (s: string, p?: unknown) => void; goBack: () => void; [key: string]: unknown } }) {
  ...
  async function handleUserClick(email: string) {
    ...
    try {
      ...
      // RPC agregada: antes esto leía producto_analytics directo, y como RLS
      // solo deja ver las propias filas a quien no es admin, cualquier usuario
      // normal veía siempre 0 vistas / 0 compartidos al tocar un contacto que
      // no fuera él mismo. La función agrega en el servidor y es accesible
      // para cualquier usuario autenticado, admin o no.
