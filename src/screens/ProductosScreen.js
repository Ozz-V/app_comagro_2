...
export default function ProductosScreen({ navigation, route }) {
  ...
  // Estados modales y navegación y precarga
  const [generandoPdf, setGenerandoPdf]   = useState(false);
  const [compartiendo, setCompartiendo]   = useState(false);
  const [pdfCache, setPdfCache]           = useState({ prodBase64: '', logoBase64: '' });
  const [logoRefreshKey, setLogoRefreshKey] = useState(''); // Se cargará de AsyncStorage
  ...
  // ─── ANALYTICS: registrar acciones en Supabase y Local ──────────────────
  async function trackAnalytics(prod, action) {
    try {
      ...
      // 2. Actualizar recientes y tendencias local instantáneo
      if (action === 'view') {
      ...
      if (user) {
        ...
        if (!error) {
          // Remover de la cola si subió bien
  ...
  useEffect(() => {
    ...
    // Precarga invisible de imágenes a Base64 para el PDF
    if (modalProd) {
      let isCurrent = true;
  ...
  async function preloadAiData(currentProducts) {
    try {
      ...
      // Correr el robot automático de IA en segundo plano solo si recibimos la lista de productos
  ...
  // Generador automático en segundo plano (Límite: 5 productos diarios por dispositivo)
  async function runAutoAiGenerator(productos, aiDict) {
    try {
      const today = new Date().toDateString();
      const lastRun = await AsyncStorage.getItem('@ai_last_auto_gen');
      if (lastRun === today) return; // Ya corrió hoy, no hacemos nada
  ...
  async function compartirPdf() {
    try {
      ...
      // Si el vendedor presiona el botón antes de que termine la precarga de fondo
      if (!finalProdB64) {
        const marcaSlug = modalProd?.marca?.toUpperCase().replace(/\s+/g, '_') || '';
        const logoUrl = `https://www.chacomer.com.py/media/wysiwyg/comagro/brands2025/${marcaSlug}.jpg?v=${logoRefreshKey}`;
  ...
  // ─── COMPARTIR COMO IMAGEN (DESDE HTML A4) ──────────
        ...
        onPress={() => {
          if (isComparing) {
            if (isSelected) {
              ...
              showAlert('Límite', 'Podés comparar hasta 4 productos a la vez.');
              return;
            } else {
          ...
          <View style={[styles.searchWrap, { flex: 1, marginBottom: 0 }]}>
            ...
            <TextInput
              ...
              onChangeText={v => {
                setBusqueda(v);
                if (v && !filtroMarca) {
                  // Búsqueda global: limpiar filtros de marca/subcategoría
                ...
                {activeTab === 'ASISTENTE' && (
                  <View style={styles.tabContent}>
                    ...
                    {loadingAi ? (
                      ...
                      <Text style={styles.aiBodyText}>{aiData || 'Texto inteligente en preparación para este producto.'}</Text>
                    )}
