...
async function initDBInternal(): Promise<SQLite.SQLiteDatabase> {
  ...
  if (tableInfo.length > 0) {
    const hasSkuColumn = tableInfo.some((col: { name: string }) => col.name === 'sku');
    const hasSearchTextColumn = tableInfo.some((col: { name: string }) => col.name === 'search_text');
    const hasSalesPitchColumn = tableInfo.some((col: { name: string }) => col.name === 'sales_pitch');
...
export async function searchProducts(marcaFiltro: string, subcatFiltro: string, textoBusqueda: string): Promise<ParsedProduct[]> {
  ...
  return results.map((r: ProductRow) => ({
...
export async function getUniqueBrands(): Promise<string[]> {
  ...
  return results.map((r: { marca: string }) => r.marca).filter(Boolean);
}
